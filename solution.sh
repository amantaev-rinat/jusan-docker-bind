#!/bin/bash
docker run -d --name jusan-docker-bind -p 7777:80 -v $(pwd)/nginx.conf:/etc/nginx/nginx.conf:ro nginx:mainline
set -euo pipefail
...
get_resp() {
    RESP=$(curl -H "Host: jusan-docker.bind" http://localhost:7777)
    echo $RESP | awk 'NF'
}
...
curl -H "Host: jusan-docker.bind" http://localhost:7777
